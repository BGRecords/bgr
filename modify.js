...
var usersRef = dbRef.ref('modifys');

//load older conatcts as well as any newly added one...
usersRef.on("child_added", function(snap) {
  $('#modifys').append(contactHtmlFromObject(snap.val()));
});
  ...
  .addEventListener("click", function( event ) {  
    ...
    if( document.querySelector('#name').value != '' 
      ...
      usersRef.push({
        ...
        spotlight: document.querySelector('#spotlight').value,
        ...
        artistkey: document.querySelector('#key').value,
        status: "Pending",
        secure: "true",
      });
      contactForm.reset();
...
function contactHtmlFromObject(contact){
  ...
  html += '<li class="list-group-item contact">';
    html += '<div>';
      ...
      html += '<hr><p>Artist Page:</p>';
      html += '<p> Spotlight Song: '+contact.spotlight+'</p>';
      html += '<p> Bio: '+contact.bio+'</p>';
